...
def start():
    ...
    blist.append(btlt)
    print (model)
    #calling the load model function to pick the trained weights
    model = Network_2048.load_model()
    print (model)
...
def process():
    ...
    if tot!=4:
        ...
        model.fit(asc,(1-alpha)*val0+alpha*(reward+gamma*val0),epochs = 5)
        close()
...
def process2():
    global a_mat
    global model
    _,maxins = Network_2048.action(model,a_mat,0)
    print(maxins)
    if maxins == 'up':
        up()
    elif maxins == 'down':
...
def close():
    global a_mat
    global status
    res = tkinter.messagebox.askyesno('status','congratulations! you have scored {} points\n Do you want to restart?'.format(int(np.max(a_mat))))
    if res is True:
        print('Restarting')
        global boardlist
        global blist
        ...
        global root
        status = Label(root, text = 'Quitting...',bd=1,relief=SUNKEN,anchor=W,font=helv12)
        status.pack(side=BOTTOM,fill=X)
...
print("\n total execution time of the program:", (finish - start))
